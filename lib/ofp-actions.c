                        ...
                        const char *arg_)
{
    ...
    while (ofputil_parse_key_value(&arg, &key, &value)) {
        if (!strcmp(key, "port")) {
        ...
        } else if(!strcmp(key, "psample")){
            VLOG_ERR("psample");
            VLOG_ERR(value);

            if (!ofputil_port_from_string(value, &output_trunc->port_s)) {
                return xasprintf("output to unknown truncate port: %s",
                                  value);
            }
            ...
            err = str_to_u32(value, &output_trunc->prob);
             ...
             enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    ...
    } else if (strstr(arg, "port") && strstr(arg, "sample")  && strstr(arg, "max_len")) {
...
OFP_ASSERT(sizeof(struct nx_action_output_trunc) == 32);
                            ...
                            struct ofpbuf *out)
{
    ...
    output_trunc->port_s = u16_to_ofp(ntohs(natrc->port_s));
    output_trunc->prob   = ntohl(natrc->prob);
                  ...
                  struct ofpbuf *out)
{
    ...
    natrc->port_s = htons(ofp_to_u16(output_trunc->port_s));
    natrc->prob = htonl(output_trunc->prob);
...
{
     ds_put_format(s, "%soutput%s(port=%"PRIu16",psample=%"PRIu16",prob=%"PRIu32",max_len=%"PRIu32")",
                   colors.special, colors.end, a->port, a->port_s, a->prob, a->max_len);
}
